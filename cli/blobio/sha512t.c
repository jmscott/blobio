/*
 *  Synopsis:
 *	openssl SHA512 client digest module interface routines.
 *  Note:
...
sha512t_init()
{
	...
	if (strcmp("roll", verb)) {
		if (!SHA512_Init(&sha512t_ctx))
			return "SHA512_Init() failed";
...
chew(unsigned char *chunk, int size)
{
	...
	if (!SHA512_Update(&sha512t_ctx, chunk, size))
		return "SHA512_Update(chunk) failed";
	/*
	 *  Copy current digest state to a temporary state,
	 *  finalize and then compare to expected state.
	 */
	tmp_ctx = sha512t_ctx;
	if (!SHA512_Final(tmp_digest, &tmp_ctx))
		return "SHA512_Final(tmp) failed";
 ...
 */
static char *
sha512t_get_update(unsigned char *src, int src_size)
{
	return chew(src, src_size);
}

static char *
sha512t_take_update(unsigned char *src, int src_size)
{
	return chew(src, src_size);
}

static char *
sha512t_took(char *reply)
{
	(void)reply;
	return 0;
}
 ...
 */
static char *
sha512t_put_update(unsigned char *src, int src_size)
{
	return chew(src, src_size);
}

static char *
sha512t_give_update(unsigned char *src, int src_size)
{
	return sha512t_put_update(src, src_size);
}

static char *
sha512t_gave(char *reply)
{
	(void)reply;
	return "0";
}
...
/*
 *  SHA512 digest is 40 characters of 0-9 or a-f.
 */
static int
sha512t_syntax()
{
...
sha512t_eat_input()
{
	...
	_TRACE("request to sha512t_eat_input()");

	while ((nread = uni_read(input_fd, buf, sizeof buf)) > 0)
		if (!SHA512_Update(&sha512t_ctx, buf, nread))
			return "SHA512_Update(chunk) failed";
	if (nread < 0)
		return strerror(errno);
	if (!SHA512_Final(bin_digest, &sha512t_ctx))
		return "SHA512_Final() failed";
...
static char *
sha512t_empty_digest()
{
	return empty;
}

struct digest	sha512t_digest =
{
	.algorithm	=	"sha",

	.init		=	sha512t_init,
	.get_update	=	sha512t_get_update,
	.take_update	=	sha512t_take_update,
	.took		=	sha512t_took,
	.put_update	=	sha512t_put_update,
	.give_update	=	sha512t_give_update,
	.gave		=	sha512t_gave,
	.close		=	sha512t_close,

	.eat_input	=	sha512t_eat_input,

	.syntax		=	sha512t_syntax,
	.empty		=	sha512t_empty,
	.empty_digest	=	sha512t_empty_digest,
