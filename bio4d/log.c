...
static char	log_path[MAX_FILE_PATH_LEN] = "log/bio4d-%s.log";
static char	log_path_format[] = "log/bio4d-%s.log";
...
/*
 *  Fork a bio4d-logger process that reads a stream of log messages from
 *  various processes, writes the messages to the physical log file and
 *  rotates the logs.  The physical file has already been opened.
 */
static void
fork_logger()
{
	...
	/*
	 *  In the parent process, the master bio4d server, so shutdown
	 *  the physical log file and the read end of the pipe, then
	 *  map the bio4d process level log file descriptor to the write end
	 *  of the logger pipe.
	 */
	if (pid > 0) {			/* parent, the master bio4d */
		logger_pid = pid;
		io_close(log_path_fd);
		io_close(log_pipe[0]);
		log_fd = log_pipe[1];
	...
	logged_pid = getpid();
	ps_title_set("bio4d-logger", (char *)0, (char *)0);
 ...
 *  Can only be called from the master bio4d process.
 *
 *  Note:
